#!/usr/bin/env bash
# Differential KLEE harness for policy-prune pass verification.
# For each policy_P*.json: runs KLEE baseline (no pass) and pruned (pass applied)
# with an identical spec, then compares verdicts.
# FP/FN detection: verdicts MUST match for every spec.
...
run_one() {
    ...
    clang-13 -target bpf -DKLEE_VERIFICATION -DVERIFY_INTERACTIONS \
        ...
        -fno-builtin -O0 -emit-llvm -c -g main.c -o main.bc 2>>"$logfile"

    if [[ "$mode" == "pruned" ]]; then
        opt -load "$PASS_LIB" -load-pass-plugin="$PASS_LIB" \
            -passes=policy-prune,globaldce,dce,simplifycfg \
            -policy-spec="$(pwd)/constraints.json" \
            main.bc -o main.bc >>"$logfile" 2>&1
    fi

    local ll_lines
    llvm-dis main.bc -o main.ll 2>>"$logfile"
    ll_lines=$(wc -l < main.ll)
