#!/usr/bin/env bash
# Extra sweep: try protocol/ethertype concretization on top of dPort.
set -u
cd "$(dirname "$0")"
...
BC=katran.bc
LL=katran.ll
RESULTS=sweep_extra_results.md
: > "$RESULTS"
...
CASES=(
  ...
  "dPort+protocol|irrel.dPort_proto.json"
  "ethertype+dPort+protocol|irrel.ether_dPort_proto.json"
  "all6|irrel.all6.json"
)
...
for c in "${CASES[@]}"; do
    ...
    logfile="log_extra_${label//[:+]/_}.txt"
