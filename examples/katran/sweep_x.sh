#!/usr/bin/env bash
# Sweep new (non-tuple) packet fields against katran with constraints.full.json.
set -u
cd "$(dirname "$0")"
...
CASES=(
  "ttl|irrel.x_ttl.json"
  "tot_len|irrel.x_tot_len.json"
  "frag_off|irrel.x_frag_off.json"
  "ip_id|irrel.x_ip_id.json"
  "ip_check|irrel.x_ip_check.json"
  "tcp_seq|irrel.x_tcp_seq.json"
  "tcp_ack_seq|irrel.x_tcp_ack_seq.json"
  "tcp_flags|irrel.x_tcp_flags.json"
  "tcp_window|irrel.x_tcp_window.json"
  "tcp_check|irrel.x_tcp_check.json"
  "tcp_urg_ptr|irrel.x_tcp_urg_ptr.json"
  "ip_all|irrel.x_ip_all.json"
  "tcp_all|irrel.x_tcp_all.json"
  "proto_plus_tcp_all|irrel.x_proto_plus_tcp_all.json"
  "everything|irrel.x_everything.json"
)
...
for c in "${CASES[@]}"; do
    ...
    logfile="log_x_${label//[:+]/_}.txt"
    echo ">>> $label  (log=$logfile)" >&2
