#!/usr/bin/env bash
# Continuation sweep: skip OOM-prone TCP fields (ack_seq, flags, window).
set -u
cd "$(dirname "$0")"
...
CASES=(
  ...
  "tcp_safe|irrel.x_tcp_safe.json"
  "ip_safe_plus_protocol|irrel.x_ip_safe_plus_protocol.json"
  "everything_safe|irrel.x_everything_safe.json"
)

echo "Policy: $POLICY  (outer cap=${CAP}s)  [skipping OOM-prone tcp_ack_seq/tcp_flags/tcp_window]" | tee -a "$RESULTS"
echo "" | tee -a "$RESULTS"
echo "| Case | Verdict | IR lines | Completed | Partial | Instructions | Wall ms | KLEE errors |" | tee -a "$RESULTS"
echo "|---|---|---:|---:|---:|---:|---:|---:|" | tee -a "$RESULTS"
for c in "${CASES[@]}"; do
    IFS='|' read -r label irrel <<<"$c"
    logfile="log_x2_${label}.txt"
