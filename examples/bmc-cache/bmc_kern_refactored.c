...
/* cache */
struct bpf_map_def SEC("maps") map_kcache = {
	.type = BPF_MAP_TYPE_ARRAY,
	.key_size = sizeof(__u32),
	.value_size = sizeof(struct bmc_cache_entry),
	.max_entries = BMC_CACHE_ENTRY_COUNT,
};
...
struct bpf_map_def SEC("maps") map_keys = {
	.type = BPF_MAP_TYPE_PERCPU_ARRAY,
	.key_size = sizeof(unsigned int),
	.value_size = sizeof(struct memcached_key),
	.max_entries = BMC_MAX_KEY_IN_PACKET,
};
...
typedef int (*tc_func_ptr)(struct __sk_buff *);

xdp_func_ptr map_progs_xdp[4];
tc_func_ptr map_progs_tc[1];
...
SEC("bmc_rx_filter")
int bmc_rx_filter_main(struct xdp_md *ctx) // safe
{
...
#pragma clang loop unroll(disable)
			...
			if (off < BMC_MAX_PACKET_LENGTH) {
				...
				// bpf_tail_call_static(ctx, &map_progs_xdp, BMC_PROG_XDP_HASH_KEYS);
				// map_progs_xdp[BMC_PROG_XDP_HASH_KEYS](ctx);
				return 0;
			}
		}
		else if (ip->protocol == IPPROTO_TCP) {
			// bpf_tail_call_static(ctx, &map_progs_xdp, BMC_PROG_XDP_INVALIDATE_CACHE);
			// map_progs_xdp[BMC_PROG_XDP_INVALIDATE_CACHE](ctx);
			return 0;
		}
	}
...
SEC("bmc_hash_keys")
int bmc_hash_keys_main(struct xdp_md *ctx) // issue
{
	void *data_end = (void *)(long)ctx->data_end;
	void *data = (void *)(long)ctx->data;
	...
	extern uint32_t bmc_hash_keys(const char *payload, unsigned int len, unsigned int *out_key_len);
	unsigned int key_len = 0;
	key->hash = bmc_hash_keys(payload, (unsigned int)((char*)data_end - payload), &key_len);
...
// 		}
	...
	// 	if (pctx->key_count > 0) {
	// 		// bpf_tail_call_static(ctx, &map_progs_xdp, BMC_PROG_XDP_PREPARE_PACKET);
	// 		// map_progs_xdp[BMC_PROG_XDP_PREPARE_PACKET](ctx);
	// 		return 0;
	// 	}
...
int bmc_prepare_packet_main(struct xdp_md *ctx) // safe
{
	...
	// bpf_tail_call_static(ctx, &map_progs_xdp, BMC_PROG_XDP_WRITE_REPLY);
	// map_progs_xdp[BMC_PROG_XDP_WRITE_REPLY](ctx);
	return 0;
...
SEC("bmc_write_reply")
int bmc_write_reply_main(struct xdp_md *ctx) // fine for now
{
	void *data_end = (void *)(long)ctx->data_end;
	void *data = (void *)(long)ctx->data;
	char *payload = (char *) data;
...
int bmc_invalidate_cache_main(struct xdp_md *ctx) // issue
{
	void *data_end = (void *)(long)ctx->data_end;
	void *data = (void *)(long)ctx->data;
	struct ethhdr *eth = data;
...
int bmc_tx_filter_main(struct __sk_buff *skb)
{
	...
	if (sport == htons(11211) && payload+5+1 <= data_end && payload[0] == 'V' && payload[1] == 'A' && payload[2] == 'L'
		...
		bpf_tail_call_static(skb, &map_progs_tc, BMC_PROG_TC_UPDATE_CACHE);
	}
