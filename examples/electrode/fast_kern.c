...
struct bpf_map_def SEC("maps") map_quorum = {
    .type = BPF_MAP_TYPE_ARRAY,
    .key_size = sizeof(__u32),
    .value_size = sizeof(struct paxos_quorum),
    .max_entries = QUORUM_BITSET_ENTRY,
};

struct bpf_map_def SEC("maps") batch_context = {
    .type = BPF_MAP_TYPE_ARRAY,
    .key_size = sizeof(__u32),
    .value_size = sizeof(struct paxos_batch),
    .max_entries = 1,
};
...
int HandleRequest_main(struct xdp_md *ctx)
{
    ...
    if (payload + MAGIC_LEN + sizeof(__u64) + REQUEST_TYPE_LEN + FAST_PAXOS_DATA_LEN > data_end)
        return XDP_PASS;
...
int HandlePrepare_main(struct xdp_md *ctx)
{
    ...
    if (payload + MAGIC_LEN + sizeof(__u64) + PREPARE_TYPE_LEN + FAST_PAXOS_DATA_LEN > data_end)
        return XDP_PASS;
...
int PrepareFastReply_main(struct xdp_md *ctx)
{
    ...
    if (payload + MAGIC_LEN + sizeof(__u64) + PREPARE_TYPE_LEN + FAST_PAXOS_DATA_LEN + sizeof(__u64) >= data_end)
        return XDP_PASS;
    ...
    *(__u32 *)payload = ctr_state->myIdx;
    ...
    udp->len = (payload - (char *)udp); // calc length.
    udp->check = 0;                          // computing udp checksum is not required

    ip->tot_len = (payload - (char *)udp + sizeof(struct iphdr));
    ip->saddr = ip->daddr;
